...
function CategoryImg({title, image, link}){
    return (
        <Link to={link} className="group hover:scale-[101%] transition-all duration-200 relative h-44 xl:h-48 rounded-xl overflow-hidden">
            <img src={image} loading="lazy" alt={title} className="w-full h-full object-cover brightness-75" />
            <p className="absolute bottom-5 left-5 text-white">{title}</p>
